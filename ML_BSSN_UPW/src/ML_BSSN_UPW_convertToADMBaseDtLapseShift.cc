...
static void ML_BSSN_UPW_convertToADMBaseDtLapseShift_Body(cGH const * restrict const cctkGH, int const dir, int const face, CCTK_REAL const normal[3], CCTK_REAL const tangentA[3], CCTK_REAL const tangentB[3], int const min[3], int const max[3], int const n_subblock_gfs, CCTK_REAL * restrict const subblock_gfs[])
{
    ...
    CCTK_REAL_VEC_SIZE)
  {
    ...
    CCTK_REAL_VEC dtalpL = 
      kmsub(kmadd(PDupwindNth1(&alpha[index]),beta1L,kmadd(PDupwindNth2(&alpha[index]),beta2L,kmul(PDupwindNth3(&alpha[index]),beta3L))),ToReal(LapseAdvectionCoeff),kmul(kpow(alphaL,harmonicN),kmul(ToReal(harmonicF),kmadd(ksub(AL,trKL),ToReal(LapseACoeff),trKL))));
    
    CCTK_REAL_VEC dtbetaxL = 
      kmadd(kmadd(PDupwindNth1(&beta1[index]),beta1L,kmadd(PDupwindNth2(&beta1[index]),beta2L,kmul(PDupwindNth3(&beta1[index]),beta3L))),ToReal(ShiftAdvectionCoeff),kmul(theta,kmul(kadd(Xt1L,kmadd(beta1L,kmul(eta,ToReal(BetaDriver*(-1 
      + 
      ShiftBCoeff))),kmul(ksub(B1L,Xt1L),ToReal(ShiftBCoeff)))),ToReal(ShiftGammaCoeff))));
    
    CCTK_REAL_VEC dtbetayL = 
      kmadd(kmadd(PDupwindNth1(&beta2[index]),beta1L,kmadd(PDupwindNth2(&beta2[index]),beta2L,kmul(PDupwindNth3(&beta2[index]),beta3L))),ToReal(ShiftAdvectionCoeff),kmul(theta,kmul(kadd(Xt2L,kmadd(beta2L,kmul(eta,ToReal(BetaDriver*(-1 
      + 
      ShiftBCoeff))),kmul(ksub(B2L,Xt2L),ToReal(ShiftBCoeff)))),ToReal(ShiftGammaCoeff))));
    
    CCTK_REAL_VEC dtbetazL = 
      kmadd(kmadd(PDupwindNth1(&beta3[index]),beta1L,kmadd(PDupwindNth2(&beta3[index]),beta2L,kmul(PDupwindNth3(&beta3[index]),beta3L))),ToReal(ShiftAdvectionCoeff),kmul(theta,kmul(kadd(Xt3L,kmadd(beta3L,kmul(eta,ToReal(BetaDriver*(-1 
      + 
