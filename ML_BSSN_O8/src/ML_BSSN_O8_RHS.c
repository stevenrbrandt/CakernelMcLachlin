...
void ML_BSSN_O8_RHS_Body(cGH const * restrict const cctkGH, int const dir, int const face, CCTK_REAL const normal[3], CCTK_REAL const tangentA[3], CCTK_REAL const tangentB[3], int const min[3], int const max[3], int const n_subblock_gfs, CCTK_REAL * restrict const subblock_gfs[])
{
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering ML_BSSN_O8_RHS_Body");
  }
  
  if (cctk_iteration % ML_BSSN_O8_RHS_calc_every != ML_BSSN_O8_RHS_calc_offset)
  {
    return;
  }
  ...
  LC_ENDLOOP3 (ML_BSSN_O8_RHS);
}

void ML_BSSN_O8_RHS(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  
  GenericFD_LoopOverInterior(cctkGH, &ML_BSSN_O8_RHS_Body);
}
