...
static void ML_BSSN_Host_Advect_beta1rhs_Body(cGH const * restrict const cctkGH, int const dir, int const face, CCTK_REAL const normal[3], CCTK_REAL const tangentA[3], CCTK_REAL const tangentB[3], int const imin[3], int const imax[3], int const n_subblock_gfs, CCTK_REAL * restrict const subblock_gfs[])
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  #pragma omp parallel
  LC_LOOP3VEC(ML_BSSN_Host_Advect_beta1rhs,
  ...
  LC_ENDLOOP3VEC(ML_BSSN_Host_Advect_beta1rhs);
}
