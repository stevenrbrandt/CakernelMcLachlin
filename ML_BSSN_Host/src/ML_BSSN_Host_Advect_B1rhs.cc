...
extern "C" void ML_BSSN_Host_Advect_B1rhs_SelectBCs(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  
  if (cctk_iteration % ML_BSSN_Host_Advect_B1rhs_calc_every != ML_BSSN_Host_Advect_B1rhs_calc_offset)
    return;
...
static void ML_BSSN_Host_Advect_B1rhs_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  CCTK_LOOP3STR(ML_BSSN_Host_Advect_B1rhs,
    ...
    vecimin,vecimax, CCTK_REAL_VEC_SIZE)
  {
    ...
    B1rhsL = 
      kmadd(kmsub(kmadd(beta1L,PDupwindNthAnti1B1,kmadd(beta2L,PDupwindNthAnti2B1,kmadd(beta3L,PDupwindNthAnti3B1,kmadd(PDupwindNthSymm1B1,kfabs(beta1L),kmadd(PDupwindNthSymm2B1,kfabs(beta2L),kmul(PDupwindNthSymm3B1,kfabs(beta3L))))))),ToReal(ShiftAdvectionCoeff),kmul(kmadd(beta1L,PDupwindNthAnti1Xt1,kmadd(beta2L,PDupwindNthAnti2Xt1,kmadd(beta3L,PDupwindNthAnti3Xt1,kmadd(PDupwindNthSymm1Xt1,kfabs(beta1L),kmadd(PDupwindNthSymm2Xt1,kfabs(beta2L),kmul(PDupwindNthSymm3Xt1,kfabs(beta3L))))))),ToReal(-1 
  ...
  CCTK_ENDLOOP3STR(ML_BSSN_Host_Advect_B1rhs);
}

extern "C" void ML_BSSN_Host_Advect_B1rhs(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering ML_BSSN_Host_Advect_B1rhs_Body");
  }
  ...
  GenericFD_AssertGroupStorage(cctkGH, "ML_BSSN_Host_Advect_B1rhs", 4, groups);
  
  GenericFD_EnsureStencilFits(cctkGH, "ML_BSSN_Host_Advect_B1rhs", 5, 5, 5);
  
  GenericFD_LoopOverInterior(cctkGH, ML_BSSN_Host_Advect_B1rhs_Body);
  
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving ML_BSSN_Host_Advect_B1rhs_Body");
  }
}
