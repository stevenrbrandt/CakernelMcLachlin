...
extern "C" void ML_BSSN_Host_Advect_phirhs_SelectBCs(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  
  if (cctk_iteration % ML_BSSN_Host_Advect_phirhs_calc_every != ML_BSSN_Host_Advect_phirhs_calc_offset)
    return;
...
static void ML_BSSN_Host_Advect_phirhs_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  CCTK_LOOP3STR(ML_BSSN_Host_Advect_phirhs,
  ...
  CCTK_ENDLOOP3STR(ML_BSSN_Host_Advect_phirhs);
}

extern "C" void ML_BSSN_Host_Advect_phirhs(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering ML_BSSN_Host_Advect_phirhs_Body");
  }
  ...
  GenericFD_AssertGroupStorage(cctkGH, "ML_BSSN_Host_Advect_phirhs", 3, groups);
  
  GenericFD_EnsureStencilFits(cctkGH, "ML_BSSN_Host_Advect_phirhs", 5, 5, 5);
  
  GenericFD_LoopOverInterior(cctkGH, ML_BSSN_Host_Advect_phirhs_Body);
  
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving ML_BSSN_Host_Advect_phirhs_Body");
  }
}
