...
static void HOST_ML_BSSN_enforce_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  ...
  CCTK_LOOP3(HOST_ML_BSSN_enforce,
    ...
    cctk_ash[0],cctk_ash[1],cctk_ash[2])
  {
    ...
    CCTK_REAL trAt CCTK_ATTRIBUTE_UNUSED = At11L*gtu11 + At22L*gtu22 + 
  ...
  CCTK_ENDLOOP3(HOST_ML_BSSN_enforce);
}

extern "C" void HOST_ML_BSSN_enforce(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering HOST_ML_BSSN_enforce_Body");
  }
  
  if (cctk_iteration % HOST_ML_BSSN_enforce_calc_every != HOST_ML_BSSN_enforce_calc_offset)
  {
    return;
  }
  ...
  const char* const groups[] = {
    ...
    "ML_BSSN::ML_lapse",
    "ML_BSSN::ML_metric"};
  GenericFD_AssertGroupStorage(cctkGH, "HOST_ML_BSSN_enforce", 3, groups);
  
  
  GenericFD_LoopOverEverything(cctkGH, HOST_ML_BSSN_enforce_Body);
  
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving HOST_ML_BSSN_enforce_Body");
  }
}
