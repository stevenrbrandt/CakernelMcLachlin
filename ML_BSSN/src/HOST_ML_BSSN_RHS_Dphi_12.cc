...
extern "C" void HOST_ML_BSSN_RHS_Dphi_12_SelectBCs(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  
  CCTK_INT ierr = 0;
  ierr = Boundary_SelectGroupForBC(cctkGH, CCTK_ALL_FACES, GenericFD_GetBoundaryWidth(cctkGH), -1 /* no table */, "ML_BSSN::DPDstandardNthphi12_group","flat");
  if (ierr < 0)
    CCTK_WARN(1, "Failed to register flat BC for ML_BSSN::DPDstandardNthphi12_group.");
  return;
}

static void HOST_ML_BSSN_RHS_Dphi_12_Body(cGH const * restrict const cctkGH, int const dir, int const face, CCTK_REAL const normal[3], CCTK_REAL const tangentA[3], CCTK_REAL const tangentB[3], int const imin[3], int const imax[3], int const n_subblock_gfs, CCTK_REAL * restrict const subblock_gfs[])
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  #pragma omp parallel
  CCTK_LOOP3(HOST_ML_BSSN_RHS_Dphi_12,
    ...
    cctk_lsh[0],cctk_lsh[1],cctk_lsh[2])
  {
    ...
    DPDstandardNthphi12[index] = PDstandardNth12(&phi[index]);
  }
  CCTK_ENDLOOP3(HOST_ML_BSSN_RHS_Dphi_12);
}

extern "C" void HOST_ML_BSSN_RHS_Dphi_12(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering HOST_ML_BSSN_RHS_Dphi_12_Body");
  }
  
  if (cctk_iteration % HOST_ML_BSSN_RHS_Dphi_12_calc_every != HOST_ML_BSSN_RHS_Dphi_12_calc_offset)
  {
    return;
  }
  ...
  GenericFD_AssertGroupStorage(cctkGH, "HOST_ML_BSSN_RHS_Dphi_12", 2, groups);
  
  GenericFD_EnsureStencilFits(cctkGH, "HOST_ML_BSSN_RHS_Dphi_12", 4, 4, 4);
  
  GenericFD_LoopOverInterior(cctkGH, HOST_ML_BSSN_RHS_Dphi_12_Body);
  
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving HOST_ML_BSSN_RHS_Dphi_12_Body");
  }
}
