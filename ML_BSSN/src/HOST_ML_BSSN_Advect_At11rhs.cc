...
extern "C" void HOST_ML_BSSN_Advect_At11rhs_SelectBCs(CCTK_ARGUMENTS)
{
...
static void HOST_ML_BSSN_Advect_At11rhs_Body(cGH const * restrict const cctkGH, int const dir, int const face, CCTK_REAL const normal[3], CCTK_REAL const tangentA[3], CCTK_REAL const tangentB[3], int const imin[3], int const imax[3], int const n_subblock_gfs, CCTK_REAL * restrict const subblock_gfs[])
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  #pragma omp parallel
  CCTK_LOOP3(HOST_ML_BSSN_Advect_At11rhs,
    ...
    cctk_lsh[0],cctk_lsh[1],cctk_lsh[2])
  {
    ...
    At11rhsL = At11rhsL + beta1L*PDupwindNthAnti1At11 + 
      ...
      PDupwindNthSymm1At11*fabs(beta1L) + 
      PDupwindNthSymm2At11*fabs(beta2L) + 
      PDupwindNthSymm3At11*fabs(beta3L);
    
    /* Copy local copies back to grid functions */
    At11rhs[index] = At11rhsL;
  }
  CCTK_ENDLOOP3(HOST_ML_BSSN_Advect_At11rhs);
}

extern "C" void HOST_ML_BSSN_Advect_At11rhs(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering HOST_ML_BSSN_Advect_At11rhs_Body");
  }
  
  if (cctk_iteration % HOST_ML_BSSN_Advect_At11rhs_calc_every != HOST_ML_BSSN_Advect_At11rhs_calc_offset)
  {
    return;
  }
  ...
  const char *const groups[] = {
    ...
    "ML_BSSN::ML_curvrhs",
    "ML_BSSN::ML_shift"};
  GenericFD_AssertGroupStorage(cctkGH, "HOST_ML_BSSN_Advect_At11rhs", 3, groups);
  
  GenericFD_EnsureStencilFits(cctkGH, "HOST_ML_BSSN_Advect_At11rhs", 5, 5, 5);
  
  GenericFD_LoopOverInterior(cctkGH, HOST_ML_BSSN_Advect_At11rhs_Body);
  
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving HOST_ML_BSSN_Advect_At11rhs_Body");
  }
}
