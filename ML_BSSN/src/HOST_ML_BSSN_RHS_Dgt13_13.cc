...
extern "C" void HOST_ML_BSSN_RHS_Dgt13_13_SelectBCs(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  
  CCTK_INT ierr = 0;
  ierr = Boundary_SelectGroupForBC(cctkGH, CCTK_ALL_FACES, GenericFD_GetBoundaryWidth(cctkGH), -1 /* no table */, "ML_BSSN::DPDstandardNthgt1313_group","flat");
  if (ierr < 0)
    CCTK_WARN(1, "Failed to register flat BC for ML_BSSN::DPDstandardNthgt1313_group.");
  return;
}

static void HOST_ML_BSSN_RHS_Dgt13_13_Body(cGH const * restrict const cctkGH, int const dir, int const face, CCTK_REAL const normal[3], CCTK_REAL const tangentA[3], CCTK_REAL const tangentB[3], int const imin[3], int const imax[3], int const n_subblock_gfs, CCTK_REAL * restrict const subblock_gfs[])
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  #pragma omp parallel
  CCTK_LOOP3(HOST_ML_BSSN_RHS_Dgt13_13,
    ...
    cctk_lsh[0],cctk_lsh[1],cctk_lsh[2])
  {
    ...
    DPDstandardNthgt1313[index] = PDstandardNth13(&gt13[index]);
  }
  CCTK_ENDLOOP3(HOST_ML_BSSN_RHS_Dgt13_13);
}

extern "C" void HOST_ML_BSSN_RHS_Dgt13_13(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering HOST_ML_BSSN_RHS_Dgt13_13_Body");
  }
  
  if (cctk_iteration % HOST_ML_BSSN_RHS_Dgt13_13_calc_every != HOST_ML_BSSN_RHS_Dgt13_13_calc_offset)
  {
    return;
  }
  ...
  const char *const groups[] = {
    ...
    "ML_BSSN::ML_metric"};
  GenericFD_AssertGroupStorage(cctkGH, "HOST_ML_BSSN_RHS_Dgt13_13", 2, groups);
  
  GenericFD_EnsureStencilFits(cctkGH, "HOST_ML_BSSN_RHS_Dgt13_13", 4, 4, 4);
  
  GenericFD_LoopOverInterior(cctkGH, HOST_ML_BSSN_RHS_Dgt13_13_Body);
  
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving HOST_ML_BSSN_RHS_Dgt13_13_Body");
  }
}
