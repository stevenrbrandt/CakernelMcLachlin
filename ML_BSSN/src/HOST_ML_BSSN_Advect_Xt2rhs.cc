...
extern "C" void HOST_ML_BSSN_Advect_Xt2rhs_SelectBCs(CCTK_ARGUMENTS)
{
...
static void HOST_ML_BSSN_Advect_Xt2rhs_Body(cGH const * restrict const cctkGH, int const dir, int const face, CCTK_REAL const normal[3], CCTK_REAL const tangentA[3], CCTK_REAL const tangentB[3], int const imin[3], int const imax[3], int const n_subblock_gfs, CCTK_REAL * restrict const subblock_gfs[])
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  #pragma omp parallel
  CCTK_LOOP3(HOST_ML_BSSN_Advect_Xt2rhs,
  ...
  CCTK_ENDLOOP3(HOST_ML_BSSN_Advect_Xt2rhs);
}

extern "C" void HOST_ML_BSSN_Advect_Xt2rhs(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering HOST_ML_BSSN_Advect_Xt2rhs_Body");
  }
  
  if (cctk_iteration % HOST_ML_BSSN_Advect_Xt2rhs_calc_every != HOST_ML_BSSN_Advect_Xt2rhs_calc_offset)
  {
    return;
  }
  ...
  const char *const groups[] = {
    ...
    "ML_BSSN::ML_Gammarhs",
    "ML_BSSN::ML_shift"};
  GenericFD_AssertGroupStorage(cctkGH, "HOST_ML_BSSN_Advect_Xt2rhs", 3, groups);
  
  GenericFD_EnsureStencilFits(cctkGH, "HOST_ML_BSSN_Advect_Xt2rhs", 5, 5, 5);
  
  GenericFD_LoopOverInterior(cctkGH, HOST_ML_BSSN_Advect_Xt2rhs_Body);
  
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving HOST_ML_BSSN_Advect_Xt2rhs_Body");
  }
}
